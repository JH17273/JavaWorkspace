...
public class B_Switch {
	 ...
	 * 			 break; // 코드 실행 후 switch문을 탈주하는 역할 연산자
	 *			 // break가 없다면 아래쪽으로 코드 순차적응로 실행.
	 * case 값2 : 
	 * 
	 * case 값n : 
	 * ...
	 * default : 위의 값들 중 일치하는 값이 없을 경우 실행할 코드를 기술
	 *			 //break;
	 * }
	 * 
	 * switch의 장점 -> case를 통해 내가 실행하기를 원하는 코드로 즉시 이동하기 때문에
	 * 				  여난속도가 if ~ else if 문보다 빠르다.
	 * 
	 * switch문의 사용처
	 *  - 항상 동일한 속도로 연산이 수행되어야 하는 경우
	 *    ex) 키보드 입력, 마우스 입력
	 *  - 가독성 측면에서 if ~ else if 문보다 더 뛰어난 경우
