...
public class A_For {
	 ...
	 * }
	 * 
	 *  - 초기식 : 반복문이 시작될 때 " 초기에 한 번만 실행 " 되는 구문
	 ...
	 *  1. 초기식 실행
	 *  2. 조건식 실행
	 *  3_1. 조건식의 결과가 참이라면 for문 내부 코드 실행후 4번으로 이동
	...
	public void method5() {
		// 1에서부터 매 번 달라지는 "랜덤값" 까지의 총 합계
		
		/*
		 * java.lang.Math 클래스의 random()을 호출하면
		 * 매 번 다른 실수형의 랜덤값을 얻어올 수 있다.
		 * 랜덤값의 범위 : 0.0 <= 랜덤값 < 1.0
		 * 
	...
	/* 실수까지의 합을 구할 때
	 	public void method?() {
			...
			double sum = 0;
			for(double i = 1; i <= random; i += 0.001) {
				sum += i;
		}
			System.out.println("1부터 랜덤값까지의 합 : "+sum);
		}
